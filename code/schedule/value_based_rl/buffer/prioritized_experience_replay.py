...
class PrioritizedExperienceReplay(object):  # stored as ( state, action, reward, next_state ) in SumTree
    ...
    def sample(self, n):
        ...
        weight = np.power(self.tree.capacity * sampling_probabilities, -self.PER_b)
        weight /= weight.max()

        return b_idx, minibatch, weight
    
    # Update the priorities on the tree
    def batch_update(self, tree_idx, abs_errors):
        abs_errors += [self.PER_e] * len(abs_errors)  # convert to abs and avoid 0
